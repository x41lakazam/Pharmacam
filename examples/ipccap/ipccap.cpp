...
{
    static constexpr uint32_t memorySize = 640U * 480U * 2U + 64U; // Allows storing a full frame of 16bit values + header
                          ...
                          memorySize))
    {
        ...
        do
        {
            ...
            if(nullptr != pImage)
            {
                ...
                printf("Got frame %u with dimensions (%d,%d)\n",
                       imageId,
                       imageWidth,
                       imageHeight);
                if (g_dumpFiles)
                {
                    printf("saved image \n");
                    std::string sFilename = R"(/home/debian/opgal/dump/frame_)"
