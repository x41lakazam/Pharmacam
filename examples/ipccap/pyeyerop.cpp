...
// private variables
    ...
    constexpr uint32_t memorySize = 640U * 480U * 2U + 64U; // Allows storing a full frame of 16bit values + header
...
// private functions declaration
    ...
    PyObject * libpyeyerop_open_client(t_thermapp *self, PyObject *args);
 ...
 *          NONE objecy in case of not avialible
    ...
    PyObject * libpyeyerop_get_data(t_thermapp *self, PyObject *args);

    // thermapp object functions list
    PyMethodDef thermappMethods[] = {
                                      {
                                        "open_shared_memory",
                                        (PyCFunction) libpyeyerop_open_client,
                                        METH_VARARGS,
                                        "open shared memory" },
                                      {
                                        "get_data",
                                        (PyCFunction) libpyeyerop_get_data,
                                        METH_VARARGS,
    ...
    PyTypeObject thermappType = {
                                  PyVarObject_HEAD_INIT(nullptr,
                                                        0) "libpyeyerop.thermapp", // tp_name
...
PyMODINIT_FUNC initlibpyeyerop(void)
{
    PyObject *module;

    module = Py_InitModule3("libpyeyerop",
                            moduleMethods,
...
// private functions implementation
    ...
    PyObject * libpyeyerop_get_data(t_thermapp *self, PyObject *args)
    {
        ...
        if (nullptr != pImage)
        {
            ...
            DPRINTF("Got frame %u with dimensions (%d,%d)\n",
                    self->imageId,
                    self->imageWidth,
                    self->imageHeight);
